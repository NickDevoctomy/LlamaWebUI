      ...
      <main>
        ...
        <div className="workspace">
          ...
          {section === 'Dashboard' ? (
            ...
            <ServerPanel status={status.data} runtimes={runtimes.data ?? []} selectedRuntime={runtime?.id ?? ''} onRuntime={setSelectedRuntime} onRestart={() => lifecycle.mutate('restart')} onRollback={() => rollback.mutate()} restarting={lifecycle.isPending || rollback.isPending} />
          ) : section === 'Runtimes' ? (
...
function ServerPanel({ status, runtimes, selectedRuntime, onRuntime, onRestart, onRollback, restarting }: {
  status?: Awaited<ReturnType<typeof api.serverStatus>>
  runtimes: Runtime[]
