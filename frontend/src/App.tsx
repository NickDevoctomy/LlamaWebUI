...
function DashboardPanel({ status, runtimes, profiles, models, downloads }: {
  ...
  return <section className="dashboard-grid">
    ...
    <section className="data-panel"><div className="panel-heading"><div><h2>System telemetry</h2><p>Host resources · live snapshot</p></div><Cpu size={20} /></div><div className="gauge-grid"><GaugeCard label="CPU" value={status?.system?.cpu_percent != null ? status.system.cpu_percent : null} suffix="%" /><GaugeCard label="RAM" value={status?.system ? Math.round(status.system.ram_used_bytes / status.system.ram_total_bytes * 100) : null} suffix="%" detail={status?.system ? `${formatBytes(status.system.ram_used_bytes)} / ${formatBytes(status.system.ram_total_bytes)}` : undefined} /><GaugeCard label="Disk" value={status?.system ? Math.round((1 - status.system.disk_free_bytes / status.system.disk_total_bytes) * 100) : null} suffix="%" detail={status?.system ? `${formatBytes(status.system.disk_free_bytes)} free` : undefined} /><GaugeCard label="GPU" value={status?.system?.gpu?.utilization_percent ?? null} suffix="%" detail={status?.system?.gpu ? undefined : 'Unavailable'} /><GaugeCard label="VRAM usage" value={status?.system?.gpu ? Math.round(status.system.gpu.memory_used_bytes / status.system.gpu.memory_total_bytes * 100) : null} suffix="%" detail={status?.system?.gpu ? `${formatBytes(status.system.gpu.memory_used_bytes)} / ${formatBytes(status.system.gpu.memory_total_bytes)}` : 'Unavailable'} /></div><div className="telemetry-foot"><span>Network received</span><strong>{status?.system ? formatBytes(status.system.network_received_bytes) : 'Unavailable'}</strong><span>Disk read</span><strong>{status?.system?.disk_read_bytes != null ? formatBytes(status.system.disk_read_bytes) : 'Unavailable'}</strong><span>Active downloads</span><strong>{activeDownloads}</strong></div></section>
  </section>
}
