...
| 2. Runtime manager | Advanced partial | Register/list/get/reprobe/remove; option/device capability parsing; in-use deletion guard; GitHub release asset discovery, stable-to-build resolution, digest-checked staged archive extraction, archive path hardening, capability probing, atomic promotion, and frontend release/asset selection | CUDA companion asset grouping, switching/rollback |
...
Implement CUDA companion asset grouping and runtime switching/rollback. Do not use the 93.7 GB target for routine acceptance.
