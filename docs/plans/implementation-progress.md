...
| 1. Application foundation | Advanced partial | Python package, FastAPI, settings, SQLite, Alembic, portable data directory, unified bounded event broker with sequencing/replay/reconciliation, React/Vite operator shell, visual CPU/RAM/disk/network telemetry gauges, disk-I/O counters, and nvidia-smi GPU/VRAM telemetry | Publish remaining download/runtime/profile state changes, structured/redacted logging, single-instance lock, diagnostics, static frontend packaging |
| 2. Runtime manager | Advanced partial | Register/list/get/reprobe/remove; option/device capability parsing; in-use deletion guard; GitHub release asset discovery, stable-to-build resolution, digest-checked staged archive extraction, archive path hardening, capability probing, atomic promotion, frontend release/asset selection, explicit runtime switching on restart, grouped companion archive installation, active-runtime removal protection, rollback restoration attempt, candidate preflight validation, and explicit rollback restoration failure reporting | Broader real-runtime rollback acceptance |
| 3. Local library and profiles | In progress | Profile persistence/deletion, saved-profile inspection/editing with alias changes in the editor, disabled-copy cloning with duplicate/padded-alias validation, portable JSON profile export/import, managed-root reconciliation reporting valid, invalid, and stray GGUF files, safe discovery of complete external GGUF shard sets, import of discovered complete models as disabled profiles, scalar GGUF metadata extraction, non-destructive profile dry-run validation with generated preset preview, reset-to-runtime-defaults workflow, durable logical model schema and registry with discovery reconciliation, durable model/profile relationships, missing-state retention for unseen logical models, guarded removal of unlinked missing logical models, typed Qwen options, capability validation, shard completeness, projector association in repository manifests, checksum-aware library reconciliation, deterministic atomic single/combined preset writing, validated completed-download projection, broken-profile health/provenance, profile prefill and exact re-download repair | Additional logical-model lifecycle operations |
| 4. Server lifecycle | In progress | Router state machine, validated argument vector, process-group launch, single-process supervisor, HTTP readiness polling, bounded newest-first log tail, crash observation, owned process-tree graceful/forced shutdown, serialized status/start/stop/restart/rollback API, durable run and restart-attempt history, safe port preflight, bounded crash recovery with rapid-failure suppression, native model list/load/unload/SSE APIs, lifecycle/model event publication through unified replayable `/api/events`, bounded SSE collection helper, idle SSE keepalives, structured prompt/decode/task/context telemetry, and effective launch-argument visibility | Real-runtime model-event frame capture; endpoint-level keepalive test deferred to `docs/tech-debt/`; system Dashboard telemetry |
| 5. Hugging Face and downloads | Advanced partial | Search, repository manifests, quant/shard grouping, projector association, explicit projector download opt-in, normalized Hub ETag metadata capture, revision-pinned durable jobs, in-file progress, bounded transient-transfer retries, periodic free-space checks, optional Hub SHA-256 metadata capture, staged checksum verification, ETag-aware resumable-file validation, complete download lifecycle event publication, interruptible child-process transfers, resumable pause, prompt cancel cleanup, managed artifact deletion, exact pinned re-download, staging, size verification, atomic publication, startup reconciliation, terminal-job clearing, responsive Discover and Downloads workflows | General library reconciliation |
...
- Runtimes exposes official release-tag discovery, backend-aware archive selection, digest visibility, stable-to-build identity, and explicit Download & install actions
...
- Dashboard and Server refresh live server status telemetry every two seconds while the router is ready
- Dashboard is now the default workspace with router, runtime, model, profile, activity, and timing summaries
 - Dashboard system telemetry now converts Windows Paths correctly for psutil-backed disk metrics; CPU/RAM/network/disk/GPU/VRAM gauges are available with an explicit GPU-supported flag
...
Next: ETag-aware transfer/cache validation and general library reconciliation. Download events now cover started, resumed, paused, cancelled, completed, and failed transitions, including durable state and byte totals for UI updates. Projector downloads are explicit via `include_projector`; they are never silently added. The newly requested Dashboard/system telemetry and structured server timing/task/context monitoring are recorded in the authoritative plan; they are not yet implemented. SSE endpoint-level testing remains deferred in `docs/tech-debt/`; do not use the 93.7 GB target for routine acceptance.
...
- The profile clone slice is complete and committed as `5c929d7` (`feat: add profile cloning`). It now has focused API coverage for configuration preservation, disabled-copy behavior, duplicate aliases, and padded aliases. Ruff, strict mypy, and the focused profile API tests pass in 2.2 seconds.
- The profile editor remains the single workflow for changing an alias and other launch settings; no separate rename endpoint or dialog is retained. The Profiles list now exposes an explicit Edit action, and the existing update endpoint persists alias changes with duplicate and capability validation. The clone workflow remains separate because it creates a disabled copy.
- Suggested next slice: add profile command import/export, or reprioritize local library reconciliation.
- Profile export is now implemented as a separate read-only slice: each profile row can download a versioned JSON document containing the alias, persisted configuration, and generated preset. No secrets are included.
- Profile import is now implemented from that versioned JSON format. Imported profiles are validated against the selected runtime and model path, always start disabled, reject malformed formats and duplicate aliases, and can be initiated from the Profiles page.
...
- Discovered external models can now be imported as disabled profiles after selecting an alias. The workflow remains non-destructive and requires a registered runtime; eight focused model-library tests and the production build pass.
- Profiles can now be reset to runtime defaults while retaining their alias and model path. The stopped-router reset is confirmed in the UI; eight focused profile API tests and the production build pass in 5.78 seconds.
...
- Coverage debt is now above the configured gate: 213 backend tests pass with 90.01% branch coverage in 17.90 seconds. Added deterministic application-owned tests for download planning/worker paths, artifact safety, transfer-process entrypoints, telemetry parsing, token rollback, model-event streaming, and release asset grouping.
- Profile command export is now available as a read-only `/api/profiles/{id}/command` endpoint and Profiles action. It renders the structured profile as a readable, quoted command without executing it or including router-owned secrets; focused API and renderer coverage passes.
- Profile command import is now available through a non-executing parser and Profiles workflow. Known llama-server flags map to typed configuration, unknown flags are retained as advanced options, imports are disabled by default, and malformed commands/runtime IDs return explicit validation errors.
