...
| 3. Local library and profiles | In progress | Profile persistence/deletion, saved-profile inspection/editing with alias changes in the editor, disabled-copy cloning with duplicate/padded-alias validation, read-only portable JSON profile export, typed Qwen options, capability validation, shard completeness, projector association in repository manifests, checksum-aware library reconciliation, deterministic atomic single/combined preset writing, validated completed-download projection, broken-profile health/provenance, profile prefill and exact re-download repair | General directory scanning, GGUF metadata, durable logical model records, command import, reset/dry-run validation |
