...
| 3. Local library and profiles | In progress | Profile persistence/deletion, saved-profile inspection/editing, disabled-copy cloning with duplicate/padded-alias validation, typed Qwen options, capability validation, shard completeness, projector association in repository manifests, checksum-aware library reconciliation, deterministic atomic single/combined preset writing, validated completed-download projection, broken-profile health/provenance, profile prefill and exact re-download repair | General directory scanning, GGUF metadata, durable logical model records, rename, command import/export, reset/dry-run validation |
...
- Suggested next slice: add profile rename as a dedicated workflow, or reprioritize command import/export and local library reconciliation.
